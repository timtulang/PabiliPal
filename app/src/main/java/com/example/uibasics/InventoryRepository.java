...
public class InventoryRepository {
    ...
    public void addItem(String name, double price, int stock, byte[] image, String desc) {
        executorService.execute(() -> {
            ...
            values.put(DatabaseHelperInventory.COLUMN_DESC, desc);
            db.insert(DatabaseHelperInventory.TABLE_NAME, null, values);
            db.close();
        });
    }
    public void updateItem(int id, String name, double price, int stock, byte[] image, String desc) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
    ...
    public List<InventoryItem> getItems() {
        ...
        String[] projection = {
                ...
                DatabaseHelperInventory.COLUMN_DESC
        };
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                String desc = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelperInventory.COLUMN_DESC));

                InventoryItem item = new InventoryItem(id, name, price, stock, image, desc);
                items.add(item);
    ...
    public List<CartItems> getAllProducts() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                productList.add(new CartItems(name, price, quantity, image, desc));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return productList;
    ...
    public List<CartItems> searchProducts(String query) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                byte[] image = cursor.getBlob(cursor.getColumnIndexOrThrow("image"));
                String desc = cursor.getString(cursor.getColumnIndexOrThrow("desc"));
                products.add(new CartItems(name, price, quantity, image, desc));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return products;
    ...
    public List<InventoryItem> searchProductsInventory(String query) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                byte[] image = cursor.getBlob(cursor.getColumnIndexOrThrow("image"));
                String desc = cursor.getString(cursor.getColumnIndexOrThrow("desc"));
                products.add(new InventoryItem(id, name, price, stock, image, desc));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return products;
